...
const RITUAL_CONFIGS = {
  ...
  'trading-mindset': {
    title: 'Nghi Lễ Trước Giao Dịch',
    subtitle: 'Chuẩn bị tâm thế trading bình tĩnh',
    icon: 'Sun',
    prompt: 'Viết ra mục tiêu và kỷ luật trading hôm nay...',
    actionLabel: 'Thở và tập trung năng lượng',
    completionMessage: 'Tâm thế đã sẵn sàng cho phiên giao dịch',
    gradients: {
      background: ['#0F1030', '#1A1A4A', '#2D2D5A'],
      accent: ['#FFBD59', '#FFD700'],
    },
    targetZone: 'focus',
    transformTo: 'clarity',
    breathingPattern: {
    ...
    affirmations: [
      ...
      'Tôi không để cảm xúc điều khiển',
...
const BurningPaper = ({ content, onBurnComplete }) => {
  ...
  useEffect(() => {
    ...
    return () => {
      isMounted = false;
      animation.stop();
      burnMaskAnim.stopAnimation();
      darkenAnim.stopAnimation();
      crumbleAnim.stopAnimation();
    };
...
const BurnCompletionModal = ({ visible, onClose }) => {
  ...
  useEffect(() => {
    ...
    return () => {
      isMounted = false;
      if (entryAnimation) entryAnimation.stop();
...
const ShimmerEffect = ({ width, height }) => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      currentAnimation = Animated.timing(shimmerPosition, {
        toValue: width * 2,
        duration: 6000,
        useNativeDriver: true,
      });
      ...
      currentAnimation.start(({ finished }) => {
        if (finished && isMounted) {
          innerTimer = setTimeout(animate, 1000); // Wait 1s before next shimmer
        }
      });
    };

    initialTimer = setTimeout(animate, 2000); // Start after 2s

...
const QuizBuilderScreen = ({ navigation, route }) => {
  ...
  const loadQuizData = async () => {
    try {
      ...
      Alert.alert('Lỗi', 'Không thể tải dữ liệu quiz');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      Alert.alert('Thành công', 'Đã lưu quiz');
    } catch (error) {
      console.error('[QuizBuilderScreen] save error:', error);
      Alert.alert('Lỗi', 'Không thể lưu quiz');
    } finally {
      setSaving(false);
    }
  };
  ...
  const showAddQuestionOptions = () => {
    Alert.alert(
      ...
      [
        ...
        { text: 'Trắc nghiệm (nhiều đáp án)', onPress: () => handleAddQuestion('multiple_select') },
        { text: 'Đúng/Sai', onPress: () => handleAddQuestion('true_false') },
        { text: 'Điền vào chỗ trống', onPress: () => handleAddQuestion('fill_blank') },

...
const CourseStudentsScreen = () => {
  ...
  const handleRevokeAccess = (student) => {
    alert({
      ...
      message: `Bạn có chắc muốn thu hồi quyền truy cập khóa học của ${student.user_name || student.user_email}?`,
      buttons: [
  ...
  const handleExtendAccess = (student) => {
    alert({
      ...
      message: 'Chọn thời gian gia hạn:',
      buttons: [
        { text: 'Hủy', style: 'cancel' },
        {
          text: '30 ngày',
  ...
  const extendAccess = async (student, days) => {
    try {
      ...
      alert({
        type: 'success',
        title: 'Thành công',
        message: 'Đã gia hạn quyền truy cập'
      });
      loadData();
    } catch (error) {
      alert({
        type: 'error',
        title: 'Lỗi',
        message: 'Không thể gia hạn quyền truy cập'
      });
    }
  };

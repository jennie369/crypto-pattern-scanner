...
class CourseAccessService {
  ...
  async getEnrolledCourses(userId) {
    try {
      // Fetch enrollments first (without nested relations to avoid PGRST200 errors)
      ...
      // Fetch course details for each enrollment separately
      const enrollmentsWithCourses = await Promise.all(
        enrollments.map(async (enrollment) => {
          try {
            ...
            if (courseError) {
              console.warn('[CourseAccessService] Course fetch error:', courseError);
              return { ...enrollment, course: null };
            }

            return { ...enrollment, course: courseData };
          } catch (err) {
            console.warn('[CourseAccessService] Error fetching course:', err);
            return { ...enrollment, course: null };
          }
        })
      );
  ...
  async getCourseStudents(courseId, options = {}) {
    try {
      ...
      // Fetch user profiles and progress for each enrollment separately
      const studentsWithProgress = await Promise.all(
        enrollments.map(async (enrollment) => {
          try {
            ...
            if (userError) {
              console.warn('[CourseAccessService] User fetch error:', userError);
            }
            ...
            console.warn('[CourseAccessService] Error fetching student data:', err);
            return { ...enrollment, user: null, progress_percent: 0 };
          }
        })
      );

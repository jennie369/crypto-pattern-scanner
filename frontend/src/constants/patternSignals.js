// ============================================
// 🎯 PATTERN SIGNAL DEFINITIONS - WITH TIER ACCESS
// Version: Complete with all 24 patterns
// ============================================
...
/**
 * Pattern aliases mapping - handles spaces, underscores, and common variations
 */
export const PATTERN_ALIASES = {
  ...
  // Triangles
  'TRIANGLE': 'SYMMETRICAL_TRIANGLE', // Generic triangle → Symmetrical as default
...
export const resolvePatternType = (patternType) => {
  ...
  // Last resort: try original case-insensitive
  const upperNormalized = normalized.toUpperCase();
  if (PATTERN_SIGNALS[upperNormalized]) {
 ...
 * @returns {Object} - Pattern signal info
 */
export const getPatternSignal = (patternType) => {
  // Resolve pattern type with aliases
  const resolved = resolvePatternType(patternType);

  // Return pattern info or fallback with debug info
  return PATTERN_SIGNALS[resolved] || {
    ...
    description: `Pattern "${patternType}" không tìm thấy (resolved to: "${resolved}")`,
    expectedWinRate: 50,
    avgRR: 1.0,
    bestTimeframes: ['1D'],
    goodTimeframes: ['4H'],
 ...
 */
export const getTimeframeQuality = (pattern, currentTimeframe) => {
  if (!pattern || !currentTimeframe) return 'CAUTION';

  const signalInfo = getPatternSignal(pattern.type);

  if (signalInfo.bestTimeframes?.includes(currentTimeframe)) {
    return 'BEST';
  }

  if (signalInfo.goodTimeframes?.includes(currentTimeframe)) {
    return 'GOOD';
  }

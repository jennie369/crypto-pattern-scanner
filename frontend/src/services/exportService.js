...
class ExportService {
  ...
  exportToText(messages, fileName = 'gem-chat-export') {
    if (!messages || messages.length === 0) {
      alert('Không có tin nhắn để export!');
      return;
    }

    const timestamp = new Date().toLocaleString('vi-VN');
    let textContent = `GEM MASTER CHATBOT - Chat Export\n`;
    ...
    messages.forEach((msg, idx) => {
      if (msg.type === 'user') {
        ...
        textContent += `[GEM MASTER] ${new Date(msg.timestamp).toLocaleString('vi-VN')}\n`;
        textContent += `${msg.content}\n\n`;
    ...
    textContent += `Generated by GEM Platform - https://gemplatform.io\n`;
  ...
  exportToMarkdown(messages, fileName = 'gem-chat-export') {
    ...
    const timestamp = new Date().toLocaleString('vi-VN');
    let mdContent = `# GEM MASTER CHATBOT - Chat Export\n\n`;
    ...
    mdContent += `*Generated by GEM Platform - https://gemplatform.io*\n`;
  ...
  async shareChat(messages) {
    if (!messages || messages.length === 0) {
      alert('Không có tin nhắn để share!');
      return;
    }
    ...
    const shareText = lastMessage.type === 'bot'
      ...
      : `Check out my GEM Master chat!`;

    // Check if Web Share API is supported
    if (navigator.share) {
      try {
        await navigator.share({
          title: 'GEM Master Chatbot',
          text: shareText,
  ...
  fallbackShare(text) {
    navigator.clipboard.writeText(text).then(() => {
      alert('✅ Đã copy vào clipboard! Bạn có thể paste để share.');
    }).catch(err => {
  ...
  printChat(messages) {
    if (!messages || messages.length === 0) {
      alert('Không có tin nhắn để print!');
      return;
    }
    ...
    let printContent = `
      <html>
        <head>
          <title>GEM Master Chatbot - Print</title>
          <style>
            body {
    ...
    messages.forEach(msg => {
      ...
      const typeLabel = messageType === 'user' ? '👤 User' : messageType === 'bot' ? '🔮 Gem Master' : '⚠️ System';
    ...
    printContent += `
          <div class="footer">
            ...
            <p>Generated by GEM Platform - https://gemplatform.io</p>
          </div>
        </body>
      </html>
    `;

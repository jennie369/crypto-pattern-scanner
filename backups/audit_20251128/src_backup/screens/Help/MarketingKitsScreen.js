...
export default function MarketingKitsScreen({ navigation }) {
  ...
  const handleDownload = async (material) => {
    ...
    try {
      ...
      if (downloadResult.status === 200) {
        ...
        Alert.alert('Thành công', 'Đã tải xuống và lưu vào thư viện');
      } else {
        throw new Error('Download failed');
      }
    } catch (error) {
      console.error('[MarketingKits] Download error:', error);
      Alert.alert('Lỗi', 'Không thể tải xuống. Vui lòng thử lại sau.');
    } finally {
      setDownloading(null);
    }
  };
  ...
  const handleShare = async (material) => {
    if (!hasAccess(material)) {
      Alert.alert(
        'Cần nâng cấp',
        'Bạn cần đăng ký làm Affiliate/CTV để chia sẻ nội dung này'
      );
      return;
    }
    ...
    try {
      ...
      Alert.alert('Lỗi', 'Không thể chia sẻ. Vui lòng thử lại sau.');
    }
  };

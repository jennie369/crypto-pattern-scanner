...
const QuizScreen = ({ navigation, route }) => {
  ...
  const loadQuiz = async () => {
    try {
      ...
      if (!quizData) {
        alert({ type: 'error', title: 'Lỗi', message: 'Không tìm thấy quiz cho bài học này' });
        navigation.goBack();
        return;
      }
      ...
      alert({ type: 'error', title: 'Lỗi', message: 'Không thể tải quiz' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = () => {
    ...
    if (unanswered.length > 0) {
      alert({
        ...
        message: `Còn ${unanswered.length} câu chưa trả lời. Bạn có muốn nộp bài không?`,
        buttons: [
      ...
      alert({
        ...
        message: 'Bạn có chắc muốn nộp bài không?',
        buttons: [
  ...
  const submitQuiz = async () => {
    try {
      ...
      alert({ type: 'error', title: 'Lỗi', message: 'Không thể nộp bài' });
    }
  };
  ...
  const handleTimeUp = () => {
    alert({
      ...
      buttons: [{ text: 'OK', onPress: submitQuiz }],
    });
  };

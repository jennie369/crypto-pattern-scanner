...
const CertificateScreen = ({ navigation, route }) => {
  ...
  const loadCertificate = async () => {
    try {
      ...
      if (!user?.id) {
        Alert.alert('Lỗi', 'Vui lòng đăng nhập để xem chứng chỉ');
        navigation.goBack();
        return;
      }
      ...
      if (!cert) {
        ...
        if (result.success) {
          ...
          Alert.alert('Lỗi', result.error || 'Không thể tạo chứng chỉ');
          navigation.goBack();
          return;
        }
      }
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      Alert.alert('Lỗi', 'Không thể chia sẻ chứng chỉ');
    } finally {
      setSharing(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      Alert.alert('Thành công', 'Chứng chỉ đã được lưu');
    } catch (error) {
      console.error('[CertificateScreen] handleSave error:', error);
      Alert.alert('Lỗi', 'Không thể lưu chứng chỉ');
    } finally {
      setSharing(false);
    }
  };

...
-- Check if crypto scenarios already exist
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM goal_scenarios WHERE life_area = 'crypto' LIMIT 1) THEN
    RAISE NOTICE 'Crypto scenarios already exist, skipping...';
  ELSE
    -- Insert 10 crypto scenarios
    ...
    RAISE NOTICE 'Successfully added 10 crypto goal scenarios!';
  END IF;
END $$;

-- Verify the insert
SELECT life_area, title, difficulty FROM goal_scenarios WHERE life_area = 'crypto' ORDER BY sort_order;

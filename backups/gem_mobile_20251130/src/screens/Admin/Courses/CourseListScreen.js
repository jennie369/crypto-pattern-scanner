...
const CourseListScreen = ({ navigation }) => {
  ...
  const handleTogglePublish = async (course) => {
    try {
      ...
      Alert.alert(
        'Thành công',
        newStatus ? 'Khóa học đã được xuất bản' : 'Khóa học đã chuyển sang nháp'
      );
    } catch (error) {
  ...
  const handleDuplicateCourse = async (course) => {
    try {
      ...
      Alert.alert(
        'Thành công',
        'Đã sao chép khóa học với tất cả modules và bài học',
        [
          {
            text: 'OK',
            onPress: () => fetchCourses(), // Refresh list
          }
        ]
      );
    } catch (error) {
      console.error('[CourseListScreen] duplicateCourse error:', error);
      Alert.alert('Lỗi', 'Không thể sao chép khóa học');
    }
  };

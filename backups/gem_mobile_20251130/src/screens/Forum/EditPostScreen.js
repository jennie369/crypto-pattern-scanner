...
const EditPostScreen = ({ navigation, route }) => {
  ...
  const initializeForm = () => {
    ...
    if (post.topic) {
      ...
      const foundTopic = allTopics.find(t => t.id === post.topic);
  ...
  // Remove selected media - WITH CONFIRMATION
  const handleRemoveMedia = () => {
    ...
    if (!hasContent) {
      Alert.alert(
        'Không thể xóa ảnh',
        'Bài viết cần có nội dung hoặc ảnh. Hãy nhập nội dung trước khi xóa ảnh.',
        [{ text: 'Đã hiểu' }]
      );
      return;
    }
          ...
          onPress: async () => {
            ...
            try {
              ...
              if (success) {
                ...
                Alert.alert('Lỗi', error || 'Không thể xóa bài viết. Vui lòng thử lại.');
              }
            } catch (error) {
  ...
  const handleSubmit = async () => {
    const hasContent = content.trim().length > 0;
    const hasImage = mediaUri !== null;

    // Validate: post must have at least content OR image
    if (!hasContent && !hasImage) {
      Alert.alert('Lỗi', 'Bài viết cần có nội dung hoặc ảnh');
      return;
    }
    ...
    try {
      ...
      if (imageChanged) {
        if (mediaUri && !mediaUri.startsWith('http')) {
          setUploading(true);
          console.log('[EditPost] Uploading new image:', mediaUri);
          ...
          if (uploadResult.success) {
            ...
            console.error('[EditPost] Upload failed:', uploadResult.error);
            Alert.alert('Lỗi', 'Không thể tải ảnh lên. Bài viết sẽ được lưu không có ảnh mới.');
          }
          setUploading(false);
        } else if (!mediaUri) {
          // Image was removed
      ...
      // Call update service
      const { success, error } = await forumService.updatePost(post.id, user.id, {
        title: title,
        content: body || title,
  ...
  const canSubmit = (content.trim() || mediaUri) && !submitting;

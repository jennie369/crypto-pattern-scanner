...
export function useLessonRealtime(lessonId, options = {}) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('system', {}, (status) => {
        console.log('[useLessonRealtime] System status:', status);
      })
      .subscribe((status) => {
...
/**
 * Hook for broadcasting lesson updates (Teacher side)
 * @param {string} lessonId - The lesson ID
 * @returns {object} Broadcast controls
 */
export function useLessonBroadcast(lessonId) {
  const [broadcasting, setBroadcasting] = useState(false);
  const [lastBroadcast, setLastBroadcast] = useState(null);

  const broadcast = useCallback(
    async (parsedContent, htmlContent) => {
      if (!lessonId || broadcasting) return { success: false };
      ...
      try {
        ...
        console.error('[useLessonBroadcast] Error:', err);
        return { success: false, error: err.message };
      } finally {
        setBroadcasting(false);
      }
    },
    [lessonId, broadcasting]
  );
  ...
  useEffect(() => {
    ...
    channel
      ...
      .on('presence', { event: 'join' }, ({ newPresences }) => {
        console.log('[useLessonPresence] Join:', newPresences);
      })
      .on('presence', { event: 'leave' }, ({ leftPresences }) => {
        console.log('[useLessonPresence] Leave:', leftPresences);
      })
      .subscribe(async (status) => {
        if (status === 'SUBSCRIBED') {
          // Get current user to track presence
          const { data: user } = await supabase.auth.getUser();
          if (user?.user) {
            await channel.track({
              id: user.user.id,
              email: user.user.email,
              role: 'student', // or get from profile

...
-- FIX: Post reactions error "is_seed_post" not found
-- Date: 2026-01-15
-- =====================================================
...
ALTER TABLE public.post_reactions
DROP CONSTRAINT IF EXISTS unique_user_forum_post_reaction;
ALTER TABLE public.post_reactions
ADD CONSTRAINT unique_user_forum_post_reaction UNIQUE(post_id, user_id);

ALTER TABLE public.post_reactions
DROP CONSTRAINT IF EXISTS unique_user_seed_post_reaction;
ALTER TABLE public.post_reactions
...
-- Step 8: Update the reaction counts trigger function (FIXED - uses seed_post_id, NOT is_seed_post)
...
BEGIN
  -- Get the correct IDs (use seed_post_id, NOT is_seed_post!)
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM public.profiles WHERE id = post_owner_id) THEN
    INSERT INTO public.notifications (
      ...
      post_owner_id,
      'reaction',
      COALESCE(reactor_name, 'Ai do') || ' da tha cam xuc',
      'voi bai viet cua ban',
      jsonb_build_object(
        'type', 'reaction',
...
BEGIN
  ...
  RAISE NOTICE '  - Removed is_seed_post references (does not exist in post_reactions)';
  RAISE NOTICE '  - Added seed_post_id column to post_reactions';
  RAISE NOTICE '  - Updated trigger functions to use seed_post_id';
  RAISE NOTICE '  - Recreated all triggers with fixed functions';
  RAISE NOTICE '';

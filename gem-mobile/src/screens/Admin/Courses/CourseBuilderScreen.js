...
import * as courseBuilderService from '../../../services/courseBuilderService';
import shopifyService from '../../../services/shopifyService';

// NOTE: Tier system removed - Course access is now based on Shopify product purchase
// Users gain access by purchasing the linked Shopify product
// const TIERS = ['FREE', 'TIER1', 'TIER2', 'TIER3']; // Deprecated
...
const CourseBuilderScreen = ({ navigation, route }) => {
  ...
  // tierRequired deprecated - access is now based on Shopify purchase
  const [isFreePreview, setIsFreePreview] = useState(false); // For free courses without Shopify product
  ...
  const loadShopifyProducts = async () => {
    try {
      ...
      console.log('[CourseBuilder] Loaded', products?.length || 0, 'Shopify products');
    } catch (error) {
      console.error('[CourseBuilder] Load Shopify products error:', error);
    } finally {
  ...
  const getSelectedProductInfo = () => {
    ...
    return shopifyProducts.find(p => {
      const pId = shopifyService.fromGlobalId(p.id) || p.id;
      return pId === shopifyProductId || p.id === shopifyProductId;
    });
  };
  ...
  const loadCourseData = async () => {
    try {
      ...
      // Load modules (fetch separately to avoid foreign key relationship issues)
      ...
      // Fetch lesson counts for each module separately
      const modulesWithCounts = await Promise.all(
        (modulesData || []).map(async (module) => {
          try {
            ...
            return {
              ...module,
              lessons: [{ count: error ? 0 : (count || 0) }],
            };
          } catch (err) {
            console.warn('[CourseBuilder] Lesson count error for module', module.id, err);
            return { ...module, lessons: [{ count: 0 }] };
          }
        })
      );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const courseData = {
        ...
        // tier_required: Set to FREE if no Shopify product (free course), otherwise null
        tier_required: (!shopifyProductId.trim() && isFreePreview) ? 'FREE' : null,
      ...
      if (isEditMode) {
        ...
        alert({
          ...
          buttons: [{ text: 'OK', onPress: () => navigation.replace('CourseBuilder', { mode: 'edit', courseId: newId }) }],
        });
      }
    } catch (error) {
      ...
      alert({
        type: 'error',
        title: 'Lỗi',
        message: error.message || 'Không thể lưu khóa học',
        buttons: [{ text: 'OK' }],
      });
    } finally {
      setSaving(false);
  ...
  // FIX Bug #26: Better validation and error handling
  const handleAddModule = () => {
    if (!isEditMode || !courseId) {
      alert({
        type: 'warning',
        title: 'Cần lưu khóa học trước',
        message: 'Vui lòng lưu khóa học trước khi thêm module.',
        buttons: [
          { text: 'Hủy', style: 'cancel' },
          { text: 'Lưu ngay', onPress: handleSave },
        ],
      });
      return;
    }
  ...
  const handlePreview = () => {
    if (!courseId) {
      alert({
        type: 'warning',
        title: 'Thông báo',
        message: 'Vui lòng lưu khóa học trước khi xem trước',
        buttons: [{ text: 'OK' }],
      });
      return;
    }
    ...
    try {
      ...
      if (status !== 'granted') {
        alert({
          type: 'error',
          title: 'Lỗi',
          message: 'Cần quyền truy cập thư viện ảnh để tải thumbnail',
          buttons: [{ text: 'OK' }],
        });
        return;
      }
  ...
  // FIX Bug #26: Use courseBuilderService with better error handling
  const confirmAddModule = async () => {
    if (!newModuleName.trim()) {
      alert({
        type: 'error',
        title: 'Lỗi',
        message: 'Vui lòng nhập tên module',
        buttons: [{ text: 'OK' }],
      });
      return;
    }
    ...
    try {
      const result = await courseBuilderService.addModule(courseId, {
      ...
      alert({
        type: 'error',
        title: 'Lỗi',
        message: error.message || 'Không thể thêm module',
        buttons: [{ text: 'OK' }],
      });
    }
  };
  ...
  const handleDeleteModule = (module) => {
    alert({
      type: 'warning',
      title: 'Xóa Module',
      message: `Bạn có chắc muốn xóa "${module.title}"?\nTất cả bài học trong module sẽ bị xóa.`,
      buttons: [
        { text: 'Hủy', style: 'cancel' },
        {
          text: 'Xóa',
  ...
  const handleDuplicateModule = async (module) => {
    try {
      ...
      alert({
        type: 'success',
        title: 'Thành công',
        message: 'Đã sao chép module và tất cả bài học',
        buttons: [{ text: 'OK' }],
      });
    } catch (error) {
      ...
      alert({
        type: 'error',
        title: 'Lỗi',
        message: 'Không thể sao chép module',
        buttons: [{ text: 'OK' }],
      });
    }
  };
  ...
  // Render access type selector (FREE preview or Shopify Purchase)
  const renderAccessTypeInfo = () => (
    <View style={styles.inputGroup}>
        ...
        {shopifyProductId ? (
          <>
            <View style={styles.accessInfoRow}>
              <View style={[styles.accessBadge, styles.accessBadgePaid]}>
                <Text style={styles.accessBadgeText}>YÊU CẦU MUA</Text>
              </View>
              <Text style={styles.accessInfoText}>
                User phải mua sản phẩm Shopify để truy cập
              </Text>
            </View>
            <Text style={styles.accessInfoNote}>
          ...
          <>
            <TouchableOpacity
              style={styles.freePreviewToggle}
              onPress={() => setIsFreePreview(!isFreePreview)}
            >
              <View style={[styles.checkbox, isFreePreview && styles.checkboxChecked]}>
                {isFreePreview && <Text style={styles.checkboxCheckmark}>✓</Text>}
              </View>
              <Text style={styles.freePreviewText}>Khóa học miễn phí (FREE)</Text>
            </TouchableOpacity>
            <Text style={styles.accessInfoNote}>
              {isFreePreview
    ...
    <GestureHandlerRootView style={{ flex: 1 }}>
          ...
          <SafeAreaView style={styles.safeArea}>
            ...
            >
              ...
              <View style={styles.section}>
                ...
                <View style={styles.inputGroup}>
                  <Text style={styles.inputLabel}>Shopify Product</Text>
                  <TouchableOpacity
                  ...
                  >
                    ...
                    {getSelectedProductInfo() ? (
                      <View style={styles.selectedProductInfo}>
                        {getSelectedProductInfo()?.images?.[0]?.src && (
                          <Image
                            source={{ uri: getSelectedProductInfo().images[0].src }}
                            style={styles.selectedProductImage}
                          />
                        )}
                        <Text style={styles.selectedProductText} numberOfLines={1}>
                          {getSelectedProductInfo()?.title}
                        </Text>
                      </View>
                    ) : (
                  ...
                  {shopifyProductId && (
                    <TouchableOpacity
                      ...
                      onPress={() => setShopifyProductId('')}
                    >
                      <Trash2 size={14} color={COLORS.error} />
                      <Text style={styles.clearProductText}>Bỏ chọn</Text>
                    </TouchableOpacity>
                  )}
                </View>
              </View>
      ...
      >
        <View style={styles.productPickerOverlay}>
          <View style={styles.productPickerContent}>
            <View style={styles.productPickerHeader}>
              <Text style={styles.productPickerTitle}>Chọn sản phẩm Shopify</Text>
              <TouchableOpacity
                style={styles.productPickerCloseBtn}
                onPress={() => setShowProductPicker(false)}
              >
                <Text style={styles.productPickerCloseText}>Đóng</Text>
              </TouchableOpacity>
            </View>
            ...
            {/* Products List */}
            {loadingProducts ? (
              <View style={styles.productListLoading}>
                <ActivityIndicator size="large" color={COLORS.gold} />
                <Text style={styles.productListLoadingText}>Đang tải sản phẩm...</Text>
              </View>
            ) : (
              <ScrollView style={styles.productList} showsVerticalScrollIndicator={false}>
                {getFilteredProducts().length === 0 ? (
                  <View style={styles.productListEmpty}>
                    <Text style={styles.productListEmptyText}>Không tìm thấy sản phẩm</Text>
                  </View>
                ) : (
                  <View>
                    {getFilteredProducts().map((product) => {
                      const productId = shopifyService.fromGlobalId(product.id) || product.id;
                      const isSelected = productId === shopifyProductId || product.id === shopifyProductId;
                      return (
                        <TouchableOpacity
                          key={product.id}
                        ...
                        >
                          ...
                          {isSelected && (
                            <View style={styles.productItemCheckmark}>
                              <Text style={styles.productItemCheckmarkText}>✓</Text>
                            </View>
                          )}
                        </TouchableOpacity>
                      );
...
const styles = StyleSheet.create({
  ...
  // Access Type Info (replaces Tier Selector)
  accessInfoCard: {
    backgroundColor: GLASS.background,
    borderWidth: 1,
    borderColor: GLASS.border,
  ...
  productPickerCloseBtn: {
    paddingVertical: SPACING.xs,
    paddingHorizontal: SPACING.md,
  },
  productPickerCloseText: {
    fontSize: TYPOGRAPHY.fontSize.md,
    color: COLORS.gold,
    fontWeight: TYPOGRAPHY.fontWeight.medium,
  },

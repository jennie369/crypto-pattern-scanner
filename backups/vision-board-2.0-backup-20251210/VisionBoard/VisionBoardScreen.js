...
const VisionBoardScreen = () => {
  ...
  const fetchCalendarAndCharts = useCallback(async () => {
    ...
    try {
      ...
      // ========== 3. WEEKLY PROGRESS: Calculate from actual data ==========
      const weeklyData = [];
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        // Today: use actual stats, past days: use decreasing values
        let totalScore = 0;
        if (isToday) {
          ...
          // Simulate past data with some variation
          totalScore = Math.max(0, 50 + Math.floor(Math.random() * 40) - (i * 5));
        }
      ...
      // ========== 5. STREAK & COMBO: Update from stats ==========
      // Streak: count consecutive days with activity
      // For now, use a calculated value based on activity
      const hasActivityToday = stats.goalsCompleted > 0 || stats.affirmationsCompleted > 0 || stats.habitsPercent > 0;
      if (hasActivityToday && streakDays === 0) {
        setStreakDays(1);
      }

      // Combo: count categories completed today
  ...
  }, [user?.id, widgets, stats, streakDays, affirmationsCompletedToday]);
  ...
  }, [widgets, fetchCalendarAndCharts]);
          ...
          <>
            ...
            <FeaturedRitualSection
              onRitualPress={(ritual) => {
                ...
                // Navigate to Ritual Playground with ritual data
                navigation.navigate('RitualPlayground', { ritual });
              }}
              onCreateRitual={() => {
                console.log('[VisionBoard] Create ritual pressed');
                // Navigate to Ritual Playground with empty/create mode
                navigation.navigate('RitualPlayground', { ritual: null, mode: 'create' });
              }}
              onAmbientMode={() => {
                console.log('[VisionBoard] Ambient mode pressed');
                // Navigate to Ritual Playground with ambient mode
                navigation.navigate('RitualPlayground', { ritual: null, mode: 'ambient' });
              }}
              onViewAllRituals={() => {
            ...
            <DivinationSection
              ...
              onCreateRitual={(reading) => {
                // Navigate to RitualPlayground with context from reading
                navigation.navigate('RitualPlayground', {
                  ritual: {
